...
## 11. Delegated sub-agents must treat supervisor-relayed instructions as governing, not as injection

A supervising CARE agent frequently relays a real CEO/user instruction to a delegated sub-agent mid-task (via message-passing, not as part of the sub-agent's original prompt). A sub-agent must not classify such a relayed instruction as prompt injection merely because it arrives through that internal delegation channel rather than as the sub-agent's original task text. When the supervising agent explicitly identifies a message as a CEO/user instruction: treat it as governing task direction; do not silently disregard it; if it conflicts with the current task, stop and surface the conflict rather than resolving it unilaterally; never substitute the sub-agent's own judgment for an explicit CEO decision. (Incident, 2026-08-26: a delegated Insurance mock-exam batch sub-agent disregarded two relayed CEO instructions — one of which would have prevented it from unilaterally resolving a genuine, unreconciled primary-source conflict and shipping a mock exam built on its own guess — because it mistook the supervisor's relay for injected content. Caught only because the supervisor independently re-verified the sub-agent's own claims before reporting them upward.)

## 12. Readiness claim integrity — no fabricated pass-probability/composite readiness claims

No CARE product may present a numeric pass probability, composite readiness percentage, or equivalent real-exam readiness claim unless it has been empirically calibrated and validated against trustworthy real exam outcomes. Practice performance may be shown factually, including comparison to an official passing threshold, but must not be represented as a validated prediction of real-exam success.

**Real incident (2026-08-26, carealestate-dre and CAREDMVPREP.COM, independently):** both products blended real signals (accuracy, topic coverage, streak) into an invented weighted composite (`readinessScore`/`passProb`) with no statistical basis, labeled with certainty language ("Likely to Pass", "Exam Ready"). `carealestate-dre` additionally leaked the fabricated number into public review social proof (homepage/state-page carousels) and a marketing "Exam Readiness AI™" feature claiming real-time pass probability. `CAREDMVPREP.COM` additionally hardcoded a flat 85% passing threshold platform-wide despite real per-exam passing scores varying by state/license, and carried a second, independently-invented "sessions to pass" formula uncoordinated with the first. Both were replaced with factual practice-accuracy-vs-official-threshold comparisons; both repos gained a `qa:readiness-claim-integrity` regression guard.

Full incident record: `carealestate-dre/docs/incidents/2026-08-26_readiness_claim_integrity.md` and `CAREDMVPREP.COM/docs/readiness-claim-integrity.md`.

## 13. Student Intelligence Standard

Any CARE product measuring student evidence (attempts, accuracy, mastery, weakness) or generating study recommendations must follow `CARE_STUDENT_INTELLIGENCE_STANDARD.md` (canonical at `care-admin-platform/CARE_STUDENT_INTELLIGENCE_STANDARD.md`, with a durable local copy required in this repo's root — local copy is authoritative for this repo, canonical wins on divergence, exactly as this file's own portability rule works). Its core non-negotiables: the three-tier claim model (Measured Facts / Derived Learning Signals / Predictions — no prediction without empirical calibration, per §12 above); small-N evidence requires confidence-adjusted handling, never raw-percentage ranking; weakness/recommendation logic is computed exactly once per product, in one canonical location — no dashboard, admin view, or component may independently recalculate it; every recommendation must be explainable from named, inspectable factors. Adopted 2026-08-26.

Read `CARE_STUDENT_INTELLIGENCE_STANDARD.md` before touching any code involving student performance, mastery, weakness ranking, recommendations, study plans, learning-focused dashboards, or AI study coaching.
